...
/// This class implements the setup for the floating row default icon and pressed state view
final public class RowAsView: RowBaseView {
    ...
    @IBOutlet private weak var subtitleWidthConstraint: NSLayoutConstraint!
    var contentView: UIView!
    let nibName = "RowAsView"
    ...
    private var rowType: RowType = .defaultRowIcon
    ...
    override public func reloadRow(with data: RowData) {
        ...
        switch self.rowType {
        case .defaultRow:
            return refreshDefaultRow()
        case .defaultRowIcon:
            return refreshDefaultIconRow()
        default:
            return refreshDefaultRow()
        }
    }
    ...
    func configureDefaultRowIcon(with rowContent: RowData, rowType: RowType) {
    ...
    public func setupDefaultRowIcon(with rowContent: RowData, rowType: RowType) {
        configureDefaultRowIcon(with: rowContent, rowType: rowType)
    }
    ...
    public func setFont(titlefont: UIFont?, subTitleFont: UIFont?) {
        if let title = titlefont {
            self.labelTitle.font = title
        }
        if let subTitle = subTitleFont {
            self.labelSubtitle.font = subTitle
        }
    }
...
extension RowAsView {
    fileprivate func refreshDefaultRow() {
        // Update data
        updateRowData()
        // Update Constraint
        leftIconWidthConstraint.constant = RowConstants.zero
        titleLeadingConstraint.constant = RowConstants.zero
        leftIcon.isHidden = true
        // Add accessibility
        self.accessibilityElements = [labelTitle!, labelSubtitle!, rightIcon!]
        applyAccessibility(data: self.rowData!)
    }

    fileprivate func refreshDefaultIconRow() {
        // Update data
        updateRowData()
        // Update Constraint
        leftIconWidthConstraint.constant = RowConstants.leftIconWidth
        titleLeadingConstraint.constant = RowConstants.leadingSpaceForTitle
        leftIcon.isHidden = false
        // Add accessibility
        self.accessibilityElements = [leftIcon!, labelTitle!, labelSubtitle!, rightIcon!]
        applyAccessibility(data: self.rowData!)
    }
    ...
    fileprivate func updateRowData() {
        ...
        if self.rowData?.subtitle == "" {
            subtitleWidthConstraint.constant = RowConstants.zero
        } else {
            subtitleWidthConstraint.constant = RowConstants.subtitleWidth
        }
